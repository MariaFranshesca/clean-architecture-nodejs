...
import {
  KendalBotResponse,
  KendalBotOption,
} from '../../entities/entity/kendalbot.entity';
import {
  KendalBotDto,
  removeSpacesBlank,
} from '../../entities/dto/kendalbot.dto';
...
export class KendalBotMongoDB implements KendalBotRepository {
  constructor(
    @InjectModel('Kendal') private readonly kendalModel: Model<IKendal>,
  ) {}
  ...
  async chatBot(kendalBotDto: KendalBotDto): Promise<KendalBotResponse> {
    ...
    if (kendalbasic) {
      ...
      options.forEach(item => {
        kendalresponse.options.push(
          new KendalBotOption(item.title, item.input),
        );
      });
      ...
      if (kendals.length > 0) {
        ...
        kendals.forEach(kendal => {
          kendalresponse.options.push(
            new KendalBotOption(kendal.title, kendal.input),
          );
        });
        return kendalresponse;
      }
